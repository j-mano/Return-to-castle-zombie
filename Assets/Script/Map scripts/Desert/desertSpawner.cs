...
public class desertspawner : MonoBehaviour
{
    ...
    float[] spawnZoneBottom = {1f, 16f, -13f, -8f};
    ...
    void lv2Spawner(GameObject[] enemies){
        float amountofenimes;
        float[] Spawnlocation = randomizelocation();

        amountofenimes = 4 + (dificulty * (Time.time * 0.1f));

        if(enemies.Length < amountofenimes){
            GetComponent<initateEnemies>().intatebasicargblomma(Spawnlocation[0],Spawnlocation[1]);
            GetComponent<initateEnemies>().intateondskefullblomma(Spawnlocation[0],Spawnlocation[1]);
        }
    }
    ...
    public float[] randomizelocation(){
        ...
        if(temp == 1){
            ...
            temparray[0] = Random.Range(spawnZoneRight[2], (spawnZoneRight[2] + spawnZoneRight[0]));
            temparray[1] = Random.Range(spawnZoneRight[3], spawnZoneRight[3] + spawnZoneRight[1]);
        }
        else if(temp == 3){
